...
high_level_cfg = get_cfg_defaults()
high_level_cfg.merge_from_file('src/config/experiments/1.5.1_config.yaml')
...
high_level_iqt = MultiTask(high_level_cfg).to(device)
high_level_iqt.load_state_dict(torch.load('experiments/1.5.1/models/netD_epoch162.pth'))
for parameter in high_level_iqt.parameters():
    parameter.requires_grad = False
high_level_iqt.eval()
...
for ref_imgs, dist_imgs, scores, categories, origin_scores in tqdm(dataloaders['val']):
    ...
    _, _, high_level_pred_scores = high_level_iqt(ref_imgs.view(-1, c, h, w),
                                                  dist_imgs.view(-1, c, h, w))
    high_level_pred = high_level_pred_scores.view(bs, ncrops, -1).mean(1).view(-1)
    ...
    final_pred = 0.4 * low_level_pred + 0.2 * medium_level_pred + 0.1 * high_level_pred + 0.3 * dists_pred
